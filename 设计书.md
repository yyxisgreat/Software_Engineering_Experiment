...
- ✅ **打包/解包**：将仓库内所有备份文件拼接为一个大文件保存
- ✅ **压缩/解压**：通过压缩节省备份文件存储空间
- ✅ **加密/解密**：由用户指定密码，将所有备份文件加密保存
...
- **依赖**: 无第三方库依赖
- **打包算法（两种）**：Header 模式（每个文件前写 header + 数据）
                       TOC 模式（先写数据 blob，末尾写索引表 TOC）
...
4. **新增模块最小侵入**: 导出/导入功能不破坏原有仓库结构，仅对仓库目录提供单文件封装
5. **无第三方依赖**: 仅使用 C++17 标准库和 Linux 系统调用
...
- 调用 PackageExport 模块实现仓库导出/导入

**支持命令**：
- backup <源目录> <仓库路径> [--include ...] [--exclude ...]
- estore <仓库路径> <目标目录>
- export <仓库路径> <输出包文件.sepkg> [--pack ...][--compress ...][--encrypt ...] [--password ...]
- import <包文件.sepkg> <仓库路径> [--password ...]
...
main() 
  ├─ 解析命令行参数
  ├─ 创建 Repository 对象
  ├─ backup: 创建 Repo -> Backup.execute()
  ├─ restore: Repo.loadIndex() -> Restore.execute()
  ├─ export: 调用 pkg::export_repo_to_package(repoDir, pkgFile,opt)
  └─ import: 调用 pkg::import_package_to_repo(pkgFile, repoDir, password)
```
...
**Windows 兼容说明**：Windows + MinGW 环境下不支持 lstat / utimensat / utimes / AT_FDCWD 等 POSIX 接口，因此实现做了条件编译：
- 使用 stat 代替 lstat（符号链接按目标文件处理）
- 使用 utime 代替高精度时间设置
- 权限 chmod 在 Windows 下效果有限，但不影响备份逻辑正确性
...
- 
**Windows 兼容说明**：MinGW 环境下部分宏（如 S_IFSOCK）可能不存在，判断逻辑做了兼容处理。
...
### 3.5 存储扩展模块 (`src/storage/package/`)

#### 3.5.1 package_export.h/cpp

**功能**：仓库导出/导入入口
**当前状态**: 该模块以“仓库目录”为对象进行封装：将 repoDir/index.txt 与 repoDir/data/** 全部纳入导出包。
- export_repo_to_package(repoDir, packageFile, Options)
     - 遍历 repoDir 下的所有文件
     - 读取每个文件内容
     - 可选压缩
     - 可选加密
     - 按选择的打包算法写入 .sepkg
- import_package_to_repo(packageFile, repoDir, password)
    - 读取包头（magic/version/算法信息/salt）
    - 读取 entries 或 TOC
    - 可选解压
    - 可选解密
    - 写回 repoDir（恢复 index.txt + data/）
...
**算法**：
**算法A**：Header 模式 (`pack_header.h/cpp`)
- 思想：每个文件一个 header + 数据，顺序写入
- 优点：实现简单、顺序读取方便
- 缺点：无法快速定位单个文件，只能顺序扫描

**算法B**：TOC 模式 (`pack_toc.h/cpp`)
- 思想：先写所有文件数据 blob，末尾写索引表 TOC（Table of Contents）
- 优点：可随机读取指定文件，更像真实归档格式
- 缺点：实现略复杂


#### 3.5.3 压缩/解压算法 (`compress_rle.h/cpp`)

**功能**: 采用简单 RLE 编码
**当前状态**: 接口已定义，新增功能使用 storage/package 模块实现
- 优点：实现简单、可快速验证
- 缺点：对一般文本/二进制压缩率有限，适合作为教学/实验算法

#### 3.5.4 加密/解密 (`encryptor.cpp/h`)
**算法**：
**算法A**：XOR 流加密 (`encrypt_xor.h/cpp`)
- 基于 password + salt 生成伪随机序列
- 逐字节 XOR
- 优点：实现非常简单
- 缺点：安全性弱，仅用于实验演示
**算法B**：RC4 流加密 (`encrypt_rc4.h/cpp`)
- 使用 RC4 KSA + PRGA 生成密钥流
- 与数据 XOR
- 优点：实现简洁，可实现较“真实”的加密流程
- 缺点：RC4 本身存在已知弱点，但满足实验目的
...
```
用户命令:
  ./backup-restore export <repo_path> <pkg_file>
    [--pack header|toc]
    [--compress none|rle]
    [--encrypt none|xor|rc4]
    [--password xxx]

1. main.cpp
   ├─ 解析 export 参数
   ├─ 填充 pkg::Options
   └─ 调用 pkg::export_repo_to_package()

2. export_repo_to_package()
   ├─ 生成 salt（若加密启用）
   ├─ 遍历 repoDir（recursive_directory_iterator）
   ├─ 对每个普通文件：
   │   ├─ 读取文件 bytes
   │   ├─ RLE 压缩（可选）
   │   ├─ XOR/RC4 加密（可选）
   │   └─ 保存 Entry（relPath, payload, originalSize）
   ├─ 写 package header（magic/version/算法/salt）
   └─ 根据 packAlg：
       ├─ Header 模式：pack_header_write(entries)
       └─ TOC 模式：pack_toc_write(toc + blobs)

3. 生成输出包 repo_full.sepkg
```
...
```
用户命令:
  ./backup-restore import <pkg_file> <repo_path> [--password xxx]

1. main.cpp
   ├─ 解析 import 参数（password）
   └─ 调用 pkg::import_package_to_repo()

2. import_package_to_repo()
   ├─ 读取 header（magic/version/算法/salt）
   ├─ 根据 packAlg：
   │   ├─ Header：pack_header_read()
   │   └─ TOC：pack_toc_read()
   ├─ 对每个条目：
   │   ├─ 解密（可选）
   │   ├─ 解压（可选）
   │   └─ 写回 repoDir 中的对应路径（恢复 index.txt 和 data/...）
```
---
...
#### 5.2.5 仓库导出导入模块 (`storage/package`)

- **`package_export.h/cpp`**：导出/导入实现
- **`pack_header.h/cpp`**：打包算法 A（Header）
- **`pack_toc.h/cpp`**：打包算法 B（TOC）
- **`compress_rle.h/cpp`**：RLE 压缩算法
- **`encrypt_xor.h/cpp`**：XOR 加密算法
- **`encrypt_rc4.h/cpp`**：RC4 加密算法
- **`algorithms.h`**：算法枚举/解析
- **`binary_io.h`**：二进制读写工具（LE endian）
...
repo_full.sepkg
    ...
    │
    ▼
repoDir restored (index.txt + data/**)
```
---
...
5. 当前实现中，打包/压缩/加密算法在 `storage/package/` 中以独立模块实现，未来可将其适配到 `PackStore / Compressor / Encryptor` 接口中，形成真正的策略模式扩展体系。
...
- **导出/导入错误**: 抛出异常并由 main 捕获，输出 Export FAIL/Import FAIL
...
- **打包/解包**：export/import 使用一次性读取文件到内存（适用于中小型仓库）
- **压缩/解压**：压缩与加密对每个文件独立处理，易于调试
- **加密/解密**：TOC 模式支持更快随机定位，但当前导入仍会一次性读出每个 blob
...
5. **更强压缩算法**：如 LZ4/DEFLATE（但需第三方库）
6. **更安全加密**：如 AES-GCM（需第三方库）
...
│   └── index.txt
├── target/              # 还原目标目录
└── repo_full.sepkg      # export 生成的单文件包
```
...
1. **备份测试**:
   ...
   ```
2. **导出测试（TOC + RLE + RC4）**：
```./backup-restore export test/repo test/repo_full.sepkg --pack toc --compress rle --encrypt rc4 --password 123456
   ```
3. **删除仓库并导入**:
``````./backup-restore export test/repo test/repo_full.sepkg --pack toc --compress rle --encrypt rc4 --password 123456
   ```
