...
void printUsage(const char* program_name) {
    ...
    std::cout << "  export <仓库路径> <包文件>         将仓库目录打包成单文件" << std::endl;
    std::cout << "  import <包文件> <仓库路径>         从单文件包恢复仓库目录" << std::endl;
    std::cout << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (command == "backup") {
        ...
        try {
            ...
            std::cout << "导出成功: " << pkgFile << std::endl;
            return 0;
        ...
        try {
            ...
            std::cout << "导入成功: " << repoDir << std::endl;
            return 0;
        } catch (const std::exception& e) {
